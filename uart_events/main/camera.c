...
#include "ov2640.h"
#endif
#if defined(CONFIG_OV7725_SUPPORT)
#include "ov7725.h"
#endif
#if defined(CONFIG_OV3660_SUPPORT)
#include "include/ov3660.h"
#endif
#if defined(CONFIG_NT99141_SUPPORT)
#include "include/nt99141.h"
#endif
...
esp_err_t camera_probe(const camera_config_t* config, camera_model_t* out_camera_model)
{
    ...
    //aiden_test nt99141
    ESP_LOGE(TAG, "slv_addr : %x", slv_addr);
...
#if defined (CONFIG_OV3660_SUPPORT)  || defined(CONFIG_NT99141_SUPPORT) || defined (CONFIG_OV5640_SUPPORT)
...
esp_err_t camera_init(const camera_config_t* config)
{
    ...
    if (pix_format == PIXFORMAT_GRAYSCALE) {
        ...
        if (s_state->sensor.id.PID == OV3660_PID || s_state->sensor.id.PID == NT99141_PID || s_state->sensor.id.PID == OV5640_PID) {
            if (is_hs_mode()) {
        ...
        if (s_state->sensor.id.PID != OV2640_PID && s_state->sensor.id.PID != NT99141_PID && s_state->sensor.id.PID != OV3660_PID && s_state->sensor.id.PID != OV5640_PID) {
